...
%                 legend('identification','validation');
        ...
        function FrictionIden(obj,jv1,jt1,jv2,jt2)
            ...
            [start1_idx,stop1_idx] = obj.CalcFrictionIdenIndex(jv1);
            [start2_idx,stop2_idx] = obj.CalcFrictionIdenIndex(jv2);
...
%                             27871,29480,31133,32697,34302,35834,37368];
        ...
        function [start_idx,stop_idx] = CalcFrictionIdenIndex(obj,jv)
            peak_idx = [];
            for idx=50:size(jv,1)-50
                if (jv(idx)>0) && (jv(idx+1)>0)...
                    && (jv(idx+2)>0) && (jv(idx+3)>0)...
                    && (jv(idx+4)>0) && (jv(idx+5)<=0)
                    peak_idx = [peak_idx,idx+4];
                elseif (jv(idx)<0) && (jv(idx+1)<0)...
                    && (jv(idx+2)<0) && (jv(idx+3)<0)...
                    && (jv(idx+4)<0) && (jv(idx+5)>=0)...
                    peak_idx = [peak_idx,idx+4];
                end
                if length(peak_idx)==22
                    break;
                end
            end
            peak_idx = [1,peak_idx];
            for idx=1:length(peak_idx)-1
                len = peak_idx(idx+1)-peak_idx(idx);
                start_idx(idx) = peak_idx(idx)+round(len*0.1);
                stop_idx(idx) = peak_idx(idx+1)-round(len*0.1);
            end
