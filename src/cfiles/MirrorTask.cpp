...
MirrorTask::MirrorTask(CleanRobot* rbt, double tf, double radius):BaseTask(rbt)
{
	_tf_clean = tf;
	_tf_pre = 10;
	_tf_post = 10;
...
void MirrorTask::SetTrajPos(MatrixXd corner_pos, int num_section, VectorXd q_fdb)
{
	if (_task_completed)
	{
		_task_state = 0;
	...
	if (_task_state==0)
	{
		_q_preclean = _rbt->IKSolve(via_pos.col(0), "q2first", 0);
		_pos_initial = _rbt->FKSolve(q_fdb).pos;
		...
		_q_postclean = _rbt->IKSolve(_pos_postclean, "q2first", 0);
...
VectorXd MirrorTask::RunPreCleanAction()
{
	...
	q_traj = _rbt->IKSolve(cpos, "q2first", alphavp.pos);
...
VectorXd MirrorTask::RunPostCleanAction()
{
	...
	q_traj = _rbt->IKSolve(cpos, "q2first", 0);
	CheckWorkspace(cpos, q_traj);
	_t += g_cycle_time;
